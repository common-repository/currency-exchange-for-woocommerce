    ...
    function br_get_currency_text_for_element($elements, $currency_load = false) {
        ...
        if( $currency_load !== false ) {
            ...
            foreach($elements as $element) {
                if( $element == 'text' ) {
                    $text .= '<span>' . $woo_currencies[$currency_load] . '</span>';
                } elseif( $element == 'custom' ) {
                    if( isset( $options['currency_custom'][$currency_load] ) ) {
                        $text .= '<span>' . $options['currency_custom'][$currency_load] . '</span>';
                    }
                } elseif( $element == 'flag' ) {
                    ...
                    $text .= '<span>' . get_woocommerce_currency_symbol( $currency_load ) . '</span>';
                    ...
                    $text .= '<span>' . apply_filters('br_currency_widget_do_type_of_text', '', $element, $currency_load) . '</span>';
                }
            }
            $currencies = $text;
        } else {
            foreach( $options['use_currency'] as $currency) {
                ...
                foreach($elements as $element) {
                    if( $element == 'text' ) {
                        $text .= '<span>' . $woo_currencies[$currency] . '</span>';
                    } elseif( $element == 'custom' ) {
                        if( isset( $options['currency_custom'][$currency] ) ) {
                            $text .= '<span>' . $options['currency_custom'][$currency] . '</span>';
                        }
                    } elseif( $element == 'flag' ) {
                        ...
                        $text .= '<span>' . get_woocommerce_currency_symbol( $currency ) . '</span>';
                    } elseif( $element == 'image' ) {
                        ...
                        $text .= '<span>' . apply_filters('br_currency_widget_do_type_of_text', '', $element, $currency) . '</span>';
                    }
                }
